...
const Sidebar: React.FC<SidebarProps> = ({ userRole, isOpen, onToggle }) => {
  ...
  return (
    <div
      className={`fixed left-0 top-0 z-30 h-screen border-r border-gray-200 bg-white transition-all duration-300 ${isOpen ? 'w-64' : 'w-20'}`}
    >
      <div className={`flex items-center justify-between p-6 ${!isOpen ? 'px-4' : ''}`}>
        <div className="flex items-center space-x-2">
          <h2
            className={`text-2xl font-bold text-blue-600 transition-all duration-300 ${!isOpen ? 'w-0 opacity-0' : 'opacity-100'}`}
          >
            Dhwani
          </h2>
        </div>
        <button
          onClick={onToggle}
          className={`z-50 rounded-lg p-2 transition-colors hover:bg-gray-100 ${!isOpen ? 'flex w-full justify-center' : ''}`}
        ...
        <nav className="mt-6">
          {getMenuItems().map((item) => (
            <Link
              ...
              className={`mt-3 flex transform items-center px-6 py-3 text-gray-700 transition-all duration-300 hover:scale-105 hover:bg-blue-50 hover:text-blue-600 ${!isOpen ? 'justify-center px-4' : ''}`}
